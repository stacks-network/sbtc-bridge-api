<script lang="ts">
	...
	import { truncate, explorerAddressUrl } from '$lib/utils'
...
<div class="bg-black-01 p-5 text-white">
<div class="grid grid-cols-3 gap-y-3 border-b pb-5">
	<div class="col-span-1">Network:</div>
	<div class="col-span-1 flex-grow"><div class="">{CONFIG.VITE_NETWORK}</div></div>
	<div class="col-span-1 flex justify-end">
		<div class="w-28 px-4 border rounded-lg flex cursor-pointer" on:keydown on:click={() => toggleNetwork()}>
			<span class="flex-grow ">Switch</span>
			<span class="align-baseline place-self-center">
	...
	<div class="col-span-1 ">sBTC Contract:</div>
	<div class="col-span-1 flex-grow"><a href={explorerAddressUrl(CONFIG.VITE_SBTC_CONTRACT_ID)} target="_blank">{getContractAddress()}</a></div>
	<div class="col-span-1 flex justify-end">
		<div class="cursor-pointer" on:keydown on:click={() => toggleNetwork()}>
	...
	<div class="col-span-1 ">Current Reward Cycle:</div>
	<div class="col-span-2 flex-grow">{$sbtcConfig.bcInfo?.poxInfo.rewardCycleId}</div>

	<div class="col-span-1 ">Next Cycle:</div>
	<div class="col-span-2 flex-grow">{getNextCycleBurnchainBlock()}</div>
</div>
...
<div class="flex mt-5">
	...
	<div class="grow">
		<div class=" ">Tx Mode</div>
		<div class="">{#if $sbtcConfig.userSettings?.useOpDrop}Using OP_DROP Mechanism{:else}Using OP_RETURN Mechanism{/if}</div>
	</div>
</div>
<div class="flex my-4">
	<div class="w-20">
	...
	<div class="col-span-2">
		...
		<div class="">{#if $sbtcConfig.userSettings?.debugMode}Showing advanced info{:else}Advanced info off{/if}</div>
	</div>
</div>
</div>