<script lang="ts">
	import { Dropdown, GradientButton, Chevron } from 'flowbite-svelte'
	...
	onMount(async () => {
		if (menuTarget && menuTarget.offsetLeft) {
			...
			//style = "position: absolute; top: 120px; right: 120px; transform: translate3d(" + (menuTarget.offsetLeft - 0) + "px, " + (menuTarget.offsetTop) + "px, 0px); position: absolute; inset: 0px auto auto 0px; margin: 0px; ";
		} else {
...
<GradientButton btnClass="menu-btn-account rounded-md w-36 text-center" data-placement="left-start"><span class="inline-flex menu-btn-text-account flex flex-nowrap"><Chevron>My account</Chevron></span></GradientButton>

<Dropdown class="" bind:open={dropdownOpen}>
	<div class="py-5 text-white  text-1xl bg-black-01 border border-gray-700 rounded-lg" role="tooltip" tabindex="-1" id="account">
		

		<div class="bg-gray-1000 p-3  mb-3 border-b">

			<div class="font-medium text-3xl mb-3">Addresses</div>

			<div class="w-96 flex items-center my-1 mb-3">
				<div class="w-14">
					<div class="col-span-1 rounded-full w-9 h-9 bg-blue-1000 pt-2 ps-2">
				...
				<div class="grow justify-start">{transformAddress(addresses().stxAddress)}</div>
			...
			<div class="w-96 flex items-center my-1 mb-3">
				<div class="w-14">
					<div class="col-span-1 rounded-full w-9 h-9 bg-yellow-1000 pt-1 ps-2">
						<svg width="28" height="36" viewBox="0 0 16 18" fill="none" xmlns="http://www.w3.org/2000/svg">
		...
		<div class="bg-gray-1000 p-5 border-b text-1xl my-2 py-2">

			<div class="font-medium text-3xl mb-3">Balances</div>

			<div class="w-96 flex items-center my-1 mb-3">
				<div class="grow justify-start text-gray-300">
					BTC : Cardinal
				</div>
				<div>
		...
		<div class="bg-gray-1000 p-3 text-2xl my-2 py-2">
			<div class="col-span-3 text-error-700 text-2xl"><a href="/" on:click|preventDefault={() => doLogout()}>Log out</a></div>
		</div>
