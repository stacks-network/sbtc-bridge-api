...
<div class="flex flex-col w-full items-center justify-center">
  <div class="text-left w-full">
    ...
    {/if}
    {#if readonly}
    <input placeholder={inputData.placeholder} id={inputData.field} readonly class="text-gray-600 bg-black tracking-wide font-extralight rounded-md p-3 h-12 w-full" type='text' bind:value={value} on:input={() => updater()}>
    {:else}
    <input placeholder={inputData.placeholder} id={inputData.field} class="text-black tracking-wide font-extralight rounded-md p-3 h-12 w-full" type='text' bind:value={value} on:input={() => updater()}>
    {/if}
  </div>
  <div class="text-xs mt-1 text-left flex w-full items-center justify-between">
    ...
    {:else}
      {#if inputData.hint}<div class=" grow">{inputData.hint}</div>{/if}
    {/if}
