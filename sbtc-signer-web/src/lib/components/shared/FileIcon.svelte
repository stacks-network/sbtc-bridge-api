...
const doClicked = (event:any) => {
    ...
    dispatch('clicked', { menuTarget });
}
</script>
  
<div class="flex items-center">
    <div title="show in explorer" on:keydown on:click={(event) => doClicked(event)} class="cursor-pointer h-8 w-8 rounded-md bg-black flex items-center justify-center border border-transparent hover:border-gray-900 transition duration-200">
        <svg class={clazz} viewBox="0 0 16 22" fill="none" xmlns="http://www.w3.org/2000/svg">
