...
if [ "$DEPLOYMENT" == "prod" ]; then
  ssh -i ~/.ssh/id_rsa -p $PORT bob@$SERVER "
    ...
    docker run -d -t -i --network host --name ${DOCKER_NAME} -p ${DOCKER_PORT}:3010 \
