...
import { pox4RewardSlotHolders } from '../../../lib/data/db_models.js';
...
export async function findRewardSlotByCycle(cycle:number):Promise<any> {
	const result = await pox4RewardSlotHolders.find({"cycle":cycle}).toArray();
	return result;
}

export async function findRewardSlotByAddress(address:string):Promise<any> {
	const result = await pox4RewardSlotHolders.find({"address":address}).toArray();
	return result;
}

export async function findRewardSlotByAddressMinHeight(address:string):Promise<any> {
	const result = await pox4RewardSlotHolders.find({"address":address}).sort({burn_block_height: 1}).limit(1).toArray();
	return result;
}
...
export async function findRewardSlot(address:string, slot_index:number, burn_block_height:number):Promise<any> {
	const result = await pox4RewardSlotHolders.findOne({"address":address, 'slot_index': slot_index, 'burn_block_height': burn_block_height});
	return result;
}
...
export async function saveRewardSlot(vote:any) {
	const result = await pox4RewardSlotHolders.insertOne(vote);
	return result;
}

async function updateRewardSlot(vote:any, changes: any) {
	const result = await pox4RewardSlotHolders.updateOne({
		_id: vote._id
	},
    { $set: changes});
	return result;
